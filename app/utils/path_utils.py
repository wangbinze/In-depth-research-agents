...
def resolve_path(filename: str, session_dir: Optional[str] = None) -> str:
    ...
    if "updated/" in path_str:
        ...
        resolved = Path(project_root_dir / relative_part).resolve()
        
        # 安全性校验：确保解析后的路径在 updated 目录下
        allowed_updated = (project_root_dir / "updated").resolve()
        if not (resolved.is_relative_to(allowed_updated) or resolved == allowed_updated):
            raise ValueError("安全违规：绝对路径越权访问被拒绝")
        return str(resolved)

    if not session_dir:
        resolved = path.resolve()
        if not (resolved.is_relative_to(project_root_dir) or resolved == project_root_dir):
            raise ValueError("安全违规：绝对路径越权访问被拒绝")
        return str(resolved)
    ...
    if path.is_absolute() or (os.name == "nt" and is_unix_abs):
        ...
        # 真实绝对路径且不在 session_dir 中：抛出 ValueError 拒绝越权访问
        if not (full_path.is_relative_to(session_path) or full_path == session_path):
            raise ValueError(f"安全违规：检测到试图访问工作区外的路径 '{full_path}'，访问已被拦截。")
    ...
    if session_name in parts:
        full_path = session_path / path.name
    elif parts and parts[0] == "output":
        full_path = session_path / path.name
    else:
        full_path = session_path / path

    # 校验相对路径解析后是否依然在 session_dir 中，防 ../ 等注入绕过
    full_path = full_path.resolve()
    if not (full_path.is_relative_to(session_path) or full_path == session_path):
        raise ValueError("安全违规：检测到试图访问工作区外的相对路径，访问已被拦截。")
